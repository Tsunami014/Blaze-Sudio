...
class BaseEntity(SkeletonEntity):
    ...
    def __init__(self, G, entity):
        ...
        self.friction = 0.1
        """Friction perpendicular to gravity direction (or if no gravity) (applied each frame) (in percent of current speed)"""
        self.grav_fric = 0.1
        """Friction applied in gravity direction (each frame) (in percent of current gravity strength)"""
    ...
    def apply_physics(self):
        ...
        fric = 1 - (self.friction + (self.not_hold_fric if not self.holding_any else 0))
        ...
        if g_mag != 0:
            ...
            vdot_fric = max(0, vdot - (self.grav_fric + (self.not_hold_fric if not self.holding_any else 0))*g_mag)
