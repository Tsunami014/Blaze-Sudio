...
class Relaxation:
    ...
    def update(self):
        if all(i[1] == self.joints[0][1] for i in self.joints):
            self.joints = [(joint[0], joint[1]+random.uniform(-0.1, 0.1)) for joint in self.joints] # Tiny vertical shift
        # Force parameters
        ...
        for _ in range(5):
            ...
            for i in range(1, len(self.joints)-1):
                for j in range(1, len(self.joints)-1):
                    if j in (i-1, i, i+1):  # Only non-adjacent
                        continue
            ...
            """start, end = self.joints[0], self.joints[-1]
            mid_point = ((start[0] + end[0]) / 2, (start[1] + end[1]) / 2)
            self.joints[-1] = self.joints[0] = mid_point"""
